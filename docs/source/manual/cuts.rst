...
The Benders cut instances are created and :ref:`added to the master problem <manual_master_add_cut>`.
...
- **Efficiency**:
  A :class:`CutGenerator` instance is initialized only once when the :class:`BendersSolver` is instantiated.
...
- **Statefulness**:
  Since the same :class:`CutGenerator` instance is used across all iterations, it can maintain state.
...
- **Lifecycle**:
  The :class:`CutGenerator` is instantiated when a :class:`BendersSolver` class is created.
  During the :meth:`BendersSolver.solve()` loop,
  the :meth:`CutGenerator.generate()` method is called whenever a new cut is needed.
  The :class:`CutGenerator` accesses information and solutions from the master and subproblems
  to create and return a list of :class:`Cut` instances.
...
especially when implementing advanced decomposition methods based on
...
- **Step 3**.  Implement the :meth:`CutGenerator.generate` method.
  This method is called in each iteration of the Benders algorithm and must return a list of :class:`Cut` objects.
...
Here is an example of a function-based generator to generate :class:`NoGoodFC`.
...
and the generated ``BendersCut`` is added back to the :class:`MasterProblem`.
