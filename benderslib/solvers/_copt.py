...
class Copt(SolverBase):
    ...
    def to_structured(self) -> dict:
        ...
        This makes ``Copt`` usable as the **source** of a cross-backend model exchange,
        e.g., to batch-solve subproblems built in COPT via a GPU backend such as
        ...
        for v in self.model.getVars():
            ...
            vars_.append({
                ...
                'vtype': 'I' if v.getType() in (COPT.INTEGER, COPT.BINARY) else 'C',
        ...
        for c in self.model.getConstrs():
            ...
            if c.ub == COPT.INFINITY:
                ...
                raise BendersBackendError(
                    f"Constraint has both bounds ({c.lb}, {c.ub}). Cannot determine sense/RHS.")
